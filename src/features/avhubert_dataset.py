...
def load_audio_visual(
    ...
    logger.info(
        (
            ...
            f"loaded {len(names)}, ",
            f"skipped {n_short} short and {n_long} long and {n_unaligned} unaligned, "
...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        noise_prob: float = 0,
        noise_snr: Union[int, Tuple[int, int]] = 0,
        noise_num: int = 1,
    ) -> None:
        self.label_rates = (
        ...
        logger.info(
            f"Noise wav: {noise_fn}->{len(self.noise_wav)} wav, "
            f"Prob: {self.noise_prob}, SNR: {self.noise_snr}, "
            f"Number of mixture: {self.noise_num}"
        )
    ...
    def load_video(self, video_name: str) -> np.ndarray:
        feats = load_video(os.path.join(self.audio_root, video_name))
        feats = self.transform(feats)
        feats = np.expand_dims(feats, axis=-1)
        return feats

    def select_noise(self) -> np.ndarray:
    ...
    def add_noise(self, clean_wav: np.ndarray) -> np.ndarray:
