...
def parse_args():
    ...
    argparser.add_argument(
        "--src",
        type=str,
        default="/home/khanhphd/Documents/data",
        required=False,
...
def main(args: argparse.Namespace):
    ...
    for channel_name in tqdm(
        ...
        src_path = os.path.join(args.src)
        ...
        if os.path.isdir(src_path):
            for file_name in os.listdir(src_path):
                ...
                api.upload_file(
                    ...
                    path_in_repo=os.path.join(args.dest, file_name),
                    repo_id="phdkhanh2507/testVLR",
                    repo_type="dataset",
                    commit_message=f"chore: update {args.dest} directory",
                    commit_description=f"Add {file_name}",
                    token=args.token,
                )
        else:
            api.upload_file(
                path_or_fileobj=src_path,
                path_in_repo=os.path.join(args.dest + ".zip"),
                repo_id="phdkhanh2507/testVLR",
                repo_type="dataset",
                commit_message=f"chore: update {args.dest} directory",
                commit_description=f"Add {src_path}",
                token=args.token,
            )
