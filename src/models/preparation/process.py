...
from src import AVSRDataLoader
...
vid_dataloader = AVSRDataLoader(
    modality="video", convert_gray=False
)
aud_dataloader = AVSRDataLoader(modality="audio")
...
seg_aud_len = seg_duration * 16000
try:
    video_data = vid_dataloader.load_data(video_path)
    audio_data = aud_dataloader.load_data(video_path)
    trim_vid_data, trim_aud_data = video_data, audio_data
    video_length = len(trim_vid_data)
    audio_length = trim_aud_data.size(1)
    if not (video_length == 0 or audio_length == 0):
        dst_vid_filename = video_path[:-4] + '_processed' + '.mp4'
        dst_aud_filename = video_path[:-4] + '_processed' + '.wav'
        save_vid_aud(
            dst_vid_filename,
            dst_aud_filename,
            trim_vid_data,
